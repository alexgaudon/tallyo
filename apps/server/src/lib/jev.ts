...
// The @ai-sdk/typesafe-ai provider speaks TypeSafe's native evaluation API and
// posts to `${baseURL}/systemone`. We authenticate with an OpenRouter API key,
// whose hosted Jev endpoint lives at /api/alpha/decisions, so the fetch is
// transparently rewritten. Request and response payloads are identical.
const OPENROUTER_DECISIONS_URL = "https://openrouter.ai/api/alpha/decisions";

const rewriteUrlToFallback = (url: string | URL | Request): string => {
  const original = url instanceof Request ? url.url : String(url);
  return original.endsWith("/systemone") ? OPENROUTER_DECISIONS_URL : original;
};

let cachedEvaluationModel: ReturnType<typeof createTypeSafeAi> | null = null;

const getEvaluationModel = () => {
  cachedEvaluationModel ??= createTypeSafeAi({
    apiKey: process.env.OPENROUTER_API_KEY ?? "",
    baseURL: "https://openrouter.ai/api/alpha",
    fetch: (input, init) => globalThis.fetch(rewriteUrlToFallback(input), init),
  });
  return cachedEvaluationModel;
};
...
const requestSuggestion = async (
  ...
  const result = await experimental_evaluate({
    model: getEvaluationModel().evaluationModel(
      process.env.JEV_MODEL || DEFAULT_JEV_MODEL,
    ),
  ...
  const tasks = input.transactions.map(
    async (t): Promise<[string, JevCategorySuggestion] | null> => {
      try {
        ...
        if (confidence < getConfidenceThreshold()) return null;

        const category = input.categories.find((c) => c.name === answer.choice);
        if (!category) return null;

        return [t.id, { categoryId: category.id, confidence }];
      } catch (error) {
