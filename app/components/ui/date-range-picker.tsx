...
export const DateRangePicker: FC<DateRangePickerProps> & {
  ...
  const loaderData = useRouteContext({
    from: "__root__",
  });
  ...
  const getPresetRange = (presetName: string): DateRange => {
    ...
    switch (preset.name) {
      ...
      case "allTime":
        from.setDate(1);
        from.setHours(0, 0, 0, 0);
        from.setMonth(new Date(loaderData.meta?.firstDate).getMonth());
        from.setFullYear(new Date(loaderData.meta?.firstDate).getFullYear());
        from.setHours(0, 0, 0, 0);
        break;
      case "last90":
